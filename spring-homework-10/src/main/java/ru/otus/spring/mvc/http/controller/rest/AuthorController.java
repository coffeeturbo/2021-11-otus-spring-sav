...
public class AuthorController {
    ...
    ResponseEntity<AuthorDto> create(@RequestBody AuthorDto author) {
        authorService.save(author.toDomainObject());
        return new ResponseEntity<>(author, HttpStatus.CREATED);
    }

    @PatchMapping("/api/v1/authors/{id}")
    ResponseEntity<AuthorDto> update(@RequestBody AuthorDto author, @PathVariable int id) {
        authorService.save(author.toDomainObject());
        return new ResponseEntity<>(author, HttpStatus.OK);
    }

    @GetMapping("/api/v1/authors/{id}")
    ResponseEntity<AuthorDto> getById(@PathVariable int id) {
